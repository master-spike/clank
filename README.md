...
_transition_ really is, and a per-key _timing habit_ — some keys you simply tend
to press "late", stealing time from one interval and refunding it to the next
without slowing you down at all. clank separates the two, drills you on the
transitions that are genuinely slow, and reports speed and accuracy normalized
against real English letter-pair frequencies so drilling hard material never
tanks your score.
...
intuition that a late-biased key doesn't cost you any total time.
